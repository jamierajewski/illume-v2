...
resource "openstack_compute_instance_v2" "illume-worker-nogpu-whole-v2" {

  count = 1
  name  = format("illume-worker-nogpu-whole-%02d", count.index + 1)

  flavor_name = "c16-128GB-1440-1socket"
  key_pair    = "illume-new"
  ...
  #  1296GB - ephemeral0.1 (90%)
  #  144GB - ephemeral0.2 (10%)
...
resource "openstack_networking_floatingip_v2" "illume-worker-nogpu-whole-v2" {
  pool = var.floating-ip-pool
}

resource "openstack_compute_floatingip_associate_v2" "illume-worker-nogpu-whole-v2" {
  floating_ip = openstack_networking_floatingip_v2.illume-worker-nogpu-whole-v2.address
  instance_id = openstack_compute_instance_v2.illume-worker-nogpu-whole-v2.id
}
