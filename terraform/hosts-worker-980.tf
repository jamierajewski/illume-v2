...
resource "openstack_networking_floatingip_v2" "illume-worker-980-v2" {
  pool = var.floating-ip-pool
}

resource "openstack_compute_floatingip_associate_v2" "illume-worker-980-v2" {
  floating_ip = openstack_networking_floatingip_v2.illume-worker-980-v2.address
  instance_id = openstack_compute_instance_v2.illume-worker-980-v2.id
}
