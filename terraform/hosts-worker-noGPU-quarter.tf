...
resource "openstack_compute_instance_v2" "illume-worker-nogpu-quarter-v2" {
  ...
  count = 1
  name  = format("illume-worker-nogpu-quarter-%02d", count.index + 1)
  ...
  #  252GB - ephemeral0.1 (70%)
  #  108GB - ephemeral0.2 (30%)
...
resource "openstack_networking_floatingip_v2" "illume-worker-nogpu-quarter-v2" {
  pool = var.floating-ip-pool
}

resource "openstack_compute_floatingip_associate_v2" "illume-worker-nogpu-quarter-v2" {
  floating_ip = openstack_networking_floatingip_v2.illume-worker-nogpu-quarter-v2.address
  instance_id = openstack_compute_instance_v2.illume-worker-nogpu-quarter-v2.id
}
