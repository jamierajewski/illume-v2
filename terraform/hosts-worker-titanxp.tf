...
resource "openstack_compute_instance_v2" "illume-worker-titanxp-v2" {

  count = 1
  name  = format("illume-worker-titanxp-%02d-v2", count.index + 1)
...
resource "openstack_networking_floatingip_v2" "illume-worker-titanxp-v2" {
  pool = var.floating-ip-pool
}

resource "openstack_compute_floatingip_associate_v2" "illume-worker-titanxp-v2" {
  floating_ip = openstack_networking_floatingip_v2.illume-worker-titanxp-v2.address
  instance_id = openstack_compute_instance_v2.illume-worker-titanxp-v2.id
}
